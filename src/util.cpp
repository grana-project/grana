...
const char * const BITCOIN_CONF_FILENAME = "granacoin.conf";
const char * const BITCOIN_PID_FILENAME = "granacoin.pid";
...
#else
    const char* pszModule = "granacoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Granacoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Granacoin";
#else
    // Unix
    return pathRet / ".granacoin";
#endif
#endif
}
