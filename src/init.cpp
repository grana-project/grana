...
void Shutdown()
{
    ...
    RenameThread("granacoin-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/granacoin-project/granacoin>";
    const std::string URL_WEBSITE = "<https://granacoin.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("granacoin-loadblk");
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if granacoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if granacoin is started while in a temporary directory.\n",
